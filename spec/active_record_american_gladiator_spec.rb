...
describe "ActiveRecord American Gladiator" do
  ...
  context "The Maze" do
    it "returns all users that have placed an order" do
      ...
      # Changeable Start
      active_users = User.distinct.joins(:orders)
      # Changeable End
  ...
  context "Atlasphere" do
    ...
    it "returns most popular items" do
      ...
      most_popular_items = Item.select('items.id, count(order_items.id) as count_of_items').joins(:orders).group('items.id').order('count_of_items DESC').limit(2) 
      #   Item.find(id)
      # end
      # Changeable Stop
